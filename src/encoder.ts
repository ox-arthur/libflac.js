...
export class Encoder {
	...
	private _beforeReadyHandler?: BeforeReadyHandler<
		Encoder,
		ChacheableEncoderCalls
	>;
	...
	constructor(private Flac: Flac, private _options: EncoderOptions) {
		this._id = Flac.create_libflac_encoder(
			_options.sampleRate,
			_options.channels,
			_options.bitsPerSample,
			_options.compression,
			_options.totalSamples,
			_options.verify
		);
	...
	private _init(isEncodeOgg?: boolean): void {
		if (this._id) {
			const state = isEncodeOgg
				? this.Flac.init_encoder_ogg_stream(
						this._id,
						this._onWrite,
						this._onMetaData
				  )
				: this.Flac.init_encoder_stream(
						this._id,
						this._onWrite,
						this._onMetaData
				  );
	...
	public reset(options?: EncoderResetOptions): boolean {
		if (this._id) {
			...
			let state: FLAC__StreamEncoderState =
				this.Flac.FLAC__stream_encoder_get_state(this._id);
			...
			if (state === /* FLAC__STREAM_ENCODER_UNINITIALIZED */ 1) {
				if (options) {
					...
					if (
						typeof options.verify !== "undefined" &&
						this.Flac.FLAC__stream_encoder_get_verify(this._id) !=
							/*non-exact comparision*/ this._options.verify
					) {
						this.Flac.FLAC__stream_encoder_set_verify(
							this._id,
					...
					if (typeof options.compression !== "number") {
						this.Flac.FLAC__stream_encoder_set_compression_level(
							this._id,
							this._options.compression
						);
					}
				}
				this.clearData();
	...
	public encode(
		...
		if (this._id && this._isInitialized && !this._isFinished) {
			...
			if (typeof isInterleaved === "undefined") {
				...
				isInterleaved = !(
					Array.isArray(pcmData) && pcmData[0] instanceof Int32Array
				);
			...
			if (typeof numberOfSamples === "undefined") {
				...
				const buff = isInterleaved
					? (pcmData as Int32Array)
					: (pcmData as Int32Array[])[0];
				// console.log('calculating numberOfSamples: byteNum='+byteNum+' for buffer ', buff);
				numberOfSamples =
					(buff.byteLength - buff.byteOffset) /
					((isInterleaved ? this._options.channels : 1) *
						buff.BYTES_PER_ELEMENT); // * byteNum);
			}

			if (isInterleaved) {
				// console.log('encoding interleaved ('+numberOfSamples+' samples)...');
				return !!this.Flac.FLAC__stream_encoder_process_interleaved(
					this._id,
					pcmData as Int32Array,
					numberOfSamples
				);
			...
			if (this._options.channels !== pcmData.length) {
				throw new Error(
					`Wrong number of channels: expected ${this._options.channels} but got ${pcmData.length}`
				);
			}
	...
	private _handleBeforeReady(
		funcName: ChacheableEncoderCalls,
		args: ArrayLike<any>
	): boolean {
